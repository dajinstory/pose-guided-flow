    ...
    def __init__(self, pretrained=None, inter_temp=1.0, final_temp=1.0):
        ...
        self.blocks = nn.Sequential(
            ...
            Block(squeeze=True, # (768,4,4)
                  flow_type='InvConvFlow', n_flows=16, coupling_type= 'SingleAffine', ch_in=768, ch_c=69, n_chunk=2, subnet=sub_conv(512,3), clamp=1.0, clamp_activation='GLOW',
                  split=False),
        )
        ...
        for feature_level in range(self.n_levels): # 4x(b,192,4,4)
            ...
            self.headers[feature_level].append(
                Block(squeeze=True, # (b,768,2,2)
                      flow_type='InvConvFlow', n_flows=4, coupling_type= 'SingleAffine', ch_in=768, ch_c=69, n_chunk=2, subnet=sub_conv(512,3), clamp=1.0, clamp_activation='GLOW',
                      split=False)
            )
            self.headers[feature_level].append(
                nn.Sequential(
